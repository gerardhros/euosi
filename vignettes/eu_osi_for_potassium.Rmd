...
## Austria (AT)
The poatssium supply in view of soil health can be calculated using `osi_c_potassium_at` using the following inputs:
...
Below an example is visualized how the soil health index varies in relation to variation in K-AAA. The optimum soil K threshold value for sustainable crop production is around 70 mg K/ kg, and the soil index shows that K deficiency might occur when K-AAA values decreases down to 20 mg K/kg.
...
dt <- CJ(B_LU = c('cropland'),
         A_K_AAA = seq(1,75,1))
dt[,osi_c_k := osi_c_potassium_ch(B_LU,A_K_AAA = A_K_AAA)]
...
Note that the crop codes for Germany are not yet available in `osi_crops` package table.For now, the B_LU code is used to differentiate between grassland and cropland (searching for "grass" in BLU).
...
Note that the crop codes for Sweden are not yet available in `osi_crops` package table. 
...
# make an example with varying K-AL content for maize
dt <- CJ(B_LU = c('maize'),
         A_K_AL = seq(1,150,1))
